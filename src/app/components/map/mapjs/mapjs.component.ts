...
export class MapJSComponent {
  ...
  setSpots(newMarkers: Spots) {
    ...
    this.markers.forEach(function(marker, i){
      ...
      } else if (marker.$lat !== newMarker.position.lat
              || marker.$lng !== newMarker.position.lng) {
        // Something was changed
        //  TODO: There is no updateMarker function in markerClusterer, so add and remove the marker
        this.markerClusterer.removeMarker(marker);
        marker.setPosition({lat: newMarker.position.lat, lng: newMarker.position.lng});
        // Update these for future comparasion
        marker.$lat = newMarker.position.lat;
        marker.$lng = newMarker.position.lng;
